...
Сначала: Настройки → Сведения о телефоне → Сведения о ПО → семь раз нажать
«Номер сборки». Появятся «Параметры разработчика».

### 5.1. Без кабеля — основной путь

Не нужны ни кабель, ни драйвер Samsung. Телефон и компьютер должны быть в одной Wi-Fi-сети.

1. Параметры разработчика → **Беспроводная отладка** → включить, «Разрешить» для сети.
2. **Сопряжение устройства с помощью кода** — окно покажет код и `IP:порт` сопряжения.
   Не закрывайте его: при закрытии код сгорает.
3. На компьютере:

   ```powershell
   adb pair 192.168.1.53:44003 084663    # порт и код из окна сопряжения
   adb connect 192.168.1.53:34965        # порт с ОСНОВНОГО экрана — он другой
   ```

Сопряжение делается один раз. `connect` — после каждого включения беспроводной
отладки: порт при этом меняется.

> **Телефон виден дважды** — по IP и по mDNS-имени. Gradle на этом падает с
> «more than one device», поэтому указывайте устройство явно:
> `adb -s <ip>:<порт> ...` или `$env:ANDROID_SERIAL = '<ip>:<порт>'` перед `gradlew`.

### 5.2. Через кабель

1. Включить **Отладка по USB**, подключить кабель, разрешить отладку на телефоне.
2. Проверить: `adb devices` — устройство со статусом `device`.

**Если телефон «просто заряжается».** Проверьте, видит ли его Windows хоть как-то:

```powershell
Get-PnpDevice | Where-Object InstanceId -match 'VID_04E8'   # 04E8 — Samsung
```

- **Пусто** — данные не доходят вовсе. Почти всегда это кабель только для зарядки.
  Драйвер тут ни при чём: без драйвера телефон всё равно появился бы, с ошибкой.
- **Есть, но со статусом ошибки** — нужен драйвер Samsung Android USB Driver.
...
Секреты заливаются один раз скриптом, который не показывает пароли и не
передаёт их аргументами командной строки:
...
Три шага, и только первый делается руками.

1. **Поднять версию** в `app/build.gradle.kts`: `versionName` — то, что увидит
   человек, `versionCode` — целое число, которое **обязано расти**. Android не
   поставит обновление с номером меньше или равным установленному, и скажет об
   этом невнятно.
2. Поставить тег и отправить его:

   ```powershell
   git tag -a v0.2.0 -m "Zales 0.2.0"
   git push origin v0.2.0
   ```

3. Дальше всё делает [release.yml](../.github/workflows/release.yml): собирает
   ядро, прогоняет ворота на том самом коммите, подписывает, называет файлы так,
   как обещает [INSTALL.md](INSTALL.md), и публикует с контрольными суммами.

Тег и `versionName` сверяются в CI. Тег, который расходится с версией внутри
APK, — это способ выпустить обновление, которое не встанет, и заметит это только
тот, у кого оно не встало.
